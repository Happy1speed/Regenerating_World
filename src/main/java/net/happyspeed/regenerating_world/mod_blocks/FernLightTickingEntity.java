    ...
    public static void tick(Level level, BlockPos pos, BlockState state, FernLightTickingEntity entity) {
        if (!level.isClientSide) {
            if (level.getGameTime() % 80 == 0) {
                BlockPos underPos = new BlockPos(pos.getX(), pos.getY(), pos.getZ());
                BlockPos growPos = new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ());
                if (level instanceof ServerLevel serverLevel) {
                    if (growPos.getY() < 10 && level.getBrightness(LightLayer.SKY, growPos) > 5) {
                        if (level.getBlockState(growPos).isAir() && level.random.nextInt(30) == 1) {
                            level.setBlock(growPos, Blocks.FERN.defaultBlockState(), 2);
                ...
                if (growPos.getY() < 10 && level.getBrightness(LightLayer.SKY, growPos) > 5) {
                    if (level.getBlockState(growPos).getBlock() instanceof BonemealableBlock bonemealableblock && bonemealableblock.isValidBonemealTarget(level, growPos, level.getBlockState(growPos)) && level.getBlockState(growPos).is(Blocks.FERN) && level.random.nextInt(30) == 1) {
                        if (level instanceof ServerLevel serverLevel) {
