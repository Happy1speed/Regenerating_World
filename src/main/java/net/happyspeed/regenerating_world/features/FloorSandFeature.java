...
public class FloorSandFeature extends Feature<NoneFeatureConfiguration> {
    public FloorSandFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> ctx) {
        ...
        int radiusX = 3 + random.nextInt(4); // blob radius in X
        int radiusY = 2 + random.nextInt(3); // blob radius in Y
        int radiusZ = 3 + random.nextInt(4); // blob radius in Z
            ...
            for (int dy = -radiusY; dy <= radiusY; dy++) {
                for (int dz = -radiusZ; dz <= radiusZ; dz++) {
                    ...
                    if (normX * normX + normY * normY + normZ * normZ <= 1.0 + random.nextFloat() * 0.2f) {
                        ...
                        if (pos.getY() > 62 || !current.is(Blocks.WATER)) continue;
