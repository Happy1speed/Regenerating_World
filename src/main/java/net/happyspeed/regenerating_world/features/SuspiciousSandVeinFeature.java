...
public class SuspiciousSandVeinFeature extends Feature<NoneFeatureConfiguration> {
    public SuspiciousSandVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
                ...
                for (int dz = -radiusZ; dz <= radiusZ; dz++) {
                    ...
                    if (normX * normX + normY * normY + normZ * normZ <= 1.0 + random.nextFloat() * 0.2f) {
                        ...
                        if (level.getBlockState(new BlockPos(pos.getX(), pos.getY() - 1, pos.getZ())).isAir() || level.getBlockState(new BlockPos(pos.getX(), pos.getY() - 1, pos.getZ())).is(Blocks.WATER)) {
                            ...
                            // Suspicious sand
                            level.setBlock(pos, Blocks.SUSPICIOUS_SAND.defaultBlockState(), Block.UPDATE_NONE);
