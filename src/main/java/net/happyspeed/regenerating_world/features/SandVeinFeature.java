...
public class SandVeinFeature extends Feature<NoneFeatureConfiguration> {
    public SandVeinFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
            ...
            for (int dy = -radiusY; dy <= radiusY; dy++) {
                for (int dz = -radiusZ; dz <= radiusZ; dz++) {
                    ...
                    if (normX * normX + normY * normY + normZ * normZ <= 1.0 + random.nextFloat() * 0.2f) {
                        ...
                        if (current.is(Blocks.WATER) || current.isAir()) continue;
